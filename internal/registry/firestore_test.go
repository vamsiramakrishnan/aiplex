...
func TestFirestoreInstance(t *testing.T) {
	...
	if retrieved.ID != inst.ID || retrieved.Kind != inst.Kind {
		t.Errorf("retrieved instance mismatch: got %v, want %v", retrieved, inst)
	}

	instances, err := store.ListInstances(ctx, capability.KindTool)
	if err != nil {
...
func TestFirestoreTemplate(t *testing.T) {
	...
	template := &models.Template{
		...
		Capabilities: []capability.Capability{
			{URI: "cap://tool/search@v1", Kind: capability.KindTool, Name: "search", Version: "v1", Description: "Search tool"},
		},
		Verified:  true,
		CreatedAt: time.Now(),
	}
	...
	templates, total, err := store.ListTemplates(ctx, capability.KindTool, 0, 10)
	if err != nil {
...
func TestFirestoreUserCaps(t *testing.T) {
	skipWithoutEmulator(t)
	...
	if err := store.SetUserCaps(ctx, userID, caps); err != nil {
		t.Fatalf("SetUserCaps failed: %v", err)
	}

	retrieved, err := store.GetUserCaps(ctx, userID)
	if err != nil {
		t.Fatalf("GetUserCaps failed: %v", err)
	}
	if len(retrieved) != len(caps) {
		t.Errorf("cap count mismatch: got %d, want %d", len(retrieved), len(caps))
	}

	empty, err := store.GetUserCaps(ctx, "non-existent-user")
	if err != nil {
		t.Fatalf("GetUserCaps for non-existent user failed: %v", err)
	}
	if empty != nil {
		t.Errorf("expected nil caps for non-existent user, got: %v", empty)
	}
}
