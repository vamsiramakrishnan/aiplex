...
// List returns all instances, optionally filtered by capability kind.
// GET /api/v1/instances?kind=tool
func (h *InstanceHandler) List(w http.ResponseWriter, r *http.Request) {
	kind := capability.Kind(r.URL.Query().Get("kind"))
	instances, err := h.store.ListInstances(r.Context(), kind)
	if err != nil {
...
func (h *InstanceHandler) Deploy(w http.ResponseWriter, r *http.Request) {
	...
	if req.TemplateID == "" {
		Error(w, r, http.StatusBadRequest, "BAD_REQUEST", "template_id is required")
		return
	}

	owner := extractOwner(r)

	inst, err := h.engine.Deploy(r.Context(), req.Kind, req.TemplateID, req.Config, owner, req.DisplayName)
	if err != nil {
